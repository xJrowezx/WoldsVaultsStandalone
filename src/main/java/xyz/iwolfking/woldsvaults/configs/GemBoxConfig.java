...
public class GemBoxConfig extends Config {
    ...
    protected void reset() {
        try (InputStream stream = this.getClass().getResourceAsStream("/default_configs/gem_box.json")) {
            CustomVaultConfigReader<GemBoxConfig> reader = new CustomVaultConfigReader<>();
            if(stream == null) {
                throw new IOException();
            }
            this.POOL = reader.readCustomConfig("gem_box", JsonUtils.parseJsonContentFromStream(stream), GemBoxConfig.class).POOL;
        } catch (IOException e) {
            System.out.println("Failed to read default Gem Box config...");
